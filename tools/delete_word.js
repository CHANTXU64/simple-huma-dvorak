...
// 读取 orig yaml 文件
const csvFileName = './orig.tigress.dict.yaml';
...
const filteredLines = csvLines.map((line, index) => {
  if (index === 0 || index === 1 || index === 2 || index === 3 || index === 4 || index === 5 || index === 6 || index === 7 || index === 8 || index === 9 || index === 10 || index === 11 || index === 12 || index === 13 || index === 14 || index === 15 || index === 16 || index === 17 || index === 18 || index === 19 || index === 20) {
    // 如果是表头行，直接返回
    return line;
  }
...
// 将过滤后的内容写入新的 yaml 文件
const filteredCSVFileName = './tigress.dict.yaml';
